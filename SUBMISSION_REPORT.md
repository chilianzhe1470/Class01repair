...
对基于 Flask 的用户管理 Web 应用进行了全面安全评估。该应用包含**登录认证、用户注册、用户搜索**三大核心功能模块。本次评估共发现 **13 项安全漏洞**，涵盖身份认证、会话管理、传输安全等领域。所有漏洞已在加固版本中完成修复。

**新增功能说明：**
- **注册功能**（`/register`）：支持用户通过表单注册，数据写入 SQLite 数据库
- **搜索功能**（`/search`）：支持通过用户名或邮箱模糊搜索已注册用户

> ⚠️ 注册与搜索功能中的 SQL 语句使用 f-string 字符串拼接，**故意保留了 SQL 注入漏洞**用于教学演示。
...
| **源代码** | https://github.com/chilianzhe1470/Classrepair |
| **核心功能** | 登录认证、用户注册、用户搜索 |
...
| 路由 | 方法 | 说明 |
|------|------|------|
| `GET /login` | GET | 显示登录表单 |
| `POST /login` | POST | 验证凭据（限流：每分钟 5 次） |
| `GET /logout` | GET | 清除会话并重定向 |
| `GET /` | GET | 首页（展示个人信息或登录提示） |

认证方式：用户名密码与内存中 `USERS` 字典的 pbkdf2:sha256 哈希值比对。

### 3.2 注册模块

| 路由 | 方法 | 说明 |
|------|------|------|
| `GET /register` | GET | 显示注册表单 |
| `POST /register` | POST | 提交注册数据 |

注册数据通过 **f-string 拼接 SQL** 写入 SQLite 数据库 `data/users.db`。

```python
# 特别注意：此处存在 SQL 注入漏洞（教学演示目的）
...
| 路由 | 方法 | 说明 |
|------|------|------|
| `GET /search` | GET | 按关键词搜索用户（URL 参数 `keyword`） |

搜索通过 **f-string 拼接 SQL** 进行模糊查询：
...
注册和搜索功能中的 SQL 查询**故意使用 f-string 字符串拼接**，未做任何过滤或参数化处理。
...
# 搜索 — 存在 SQL 注入
sql = f"SELECT id, username, email, phone FROM users WHERE username LIKE '%{keyword}%' OR email LIKE '%{email}%'"
```

**教学演示目的：** 学生可通过搜索框尝试 SQL 注入攻击，观察后台打印的 SQL 语句变化。例如输入 `test' OR '1'='1` 可突破查询限制。

### 5.2 数据库安全

| 方面 | 现状 | 生产建议 |
|------|------|---------|
| 密码存储 | 明文存储在 SQLite 中 | 应对密码进行哈希处理 |
| 数据库位置 | `data/users.db`（默认路径） | 应使用独立数据库服务器 |
| 访问权限 | 无认证限制 | 应配置数据库访问控制 |
...
通过 POST 提交注册表单后，新用户数据写入 SQLite 数据库。注册成功跳转至登录页并提示"注册成功，请登录"。

### 8.2 搜索验证

已登录状态下，在搜索框输入关键词可模糊匹配用户名或邮箱。后台日志输出执行的 SQL 语句：
...
6. **替换字符串拼接为参数化查询** — 消除注册和搜索功能的 SQL 注入漏洞
7. **密码哈希存储** — 注册密码应使用 `werkzeug.security` 进行哈希
8. **定期依赖扫描** — 使用 `pip-audit` 或 GitHub Dependabot 检测已知漏洞
...
新增的注册与搜索功能**有意识地保留了 SQL 注入漏洞**，作为网络安全教学演示素材。学生可以通过这些功能模块实践 SQL 注入攻击与防御技术。
